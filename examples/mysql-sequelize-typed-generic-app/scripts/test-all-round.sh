...
echo "[1/7] Checking local RBAC CLI binary..."
if [ ! -x "$RBAC_BIN" ]; then
...
echo "[3/7] Generating RBAC files with auto-detected config..."
rm -rf "$OUT_DIR"
"$RBAC_BIN" init --out "$OUT_DIR"

echo "[4/7] Validating generated manifest..."
...
echo "[5/7] Checking manifest mappings..."
...
echo "[6/7] Checking generated migration/model artifacts..."
...
echo "[7/7] Building TypeScript example..."
cd "$ROOT_DIR"
